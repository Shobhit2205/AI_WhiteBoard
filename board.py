...
class main:
    def __init__(self, master):
        ...
        self.color_bg = '#ececec'  # Background colour
        self.root.configure(background='#ececec')
        self.root.resizable(0, 0)
        self.color_frame = LabelFrame(self.root, relief=RIDGE, bg='#ececec')
        self.color_frame.place(x=0, y=0, width=90, height=210)
        self.state = 0
        self.clicked = 0
        self.clicked1 = 0

        # making colour buttons
        colors = ['blue', 'black', 'red', '#00a5f9', '#682cbf', '#009888', '#F4F27E', '#FF69B4']
        i = 7
        j = 0
        for color in colors:
            if color == 'blue':
                Button(self.color_frame, bd=3, bg=color, relief=RIDGE, width=90,
                       height=20, command=self.change_colorblue).grid(row=i, column=j)
            elif color == 'black':
                Button(self.color_frame, bd=3, bg=color, relief=RIDGE, width=90,
                       height=20, command=self.change_colorblack).grid(row=i, column=j)
            elif color == 'red':
                Button(self.color_frame, bd=3, bg='red', relief=RIDGE, width=90,
                       height=20, command=self.change_colorred).grid(row=i, column=j)
            elif color == '#00a5f9':
                Button(self.color_frame, bd=3, bg=color, relief=RIDGE, width=90,
                       height=20, command=self.change_colora).grid(row=i, column=j)
            elif color == '#682cbf':
                Button(self.color_frame, bd=3, bg=color, relief=RIDGE, width=90,
                       height=20, command=self.change_colorb).grid(row=i, column=j)
            elif color == '#009888':
                Button(self.color_frame, bd=3, bg=color, relief=RIDGE, width=90,
                       height=20, command=self.change_colorc).grid(row=i, column=j)
            elif color == '#F4F27E':
                Button(self.color_frame, bd=3, bg=color, relief=RIDGE, width=90,
                       height=20, command=self.change_colord).grid(row=i, column=j)
            elif color == '#FF69B4':
                Button(self.color_frame, bd=3, bg=color, relief=RIDGE, width=90,
                       height=20, command=self.change_colore).grid(row=i, column=j)
                       
            i = i+1

        # making other buttons and placing them in apprprite place

        self.erazer_button = Button(self.root, text='ERAZE', font=(
            'calibri', 10), bd=0, bg='white', command=self.eraze, width=90, relief=RIDGE)
        self.erazer_button.place(x=0, y=275)

        self.clear_button = Button(
            self.root, text='CLEAR', bd=0, bg='white', command=self.clear, width=90, relief=RIDGE)
        self.clear_button.place(x=0, y=245)

        self.save_button = Button(
            self.root, text='SAVE', bd=0, bg='white', command=self.save, width=90, relief=RIDGE)
        self.save_button.place(x=0, y=215)

        # A 'Listen' button is herby added
        self.recognise_button = Button(
            self.root, text='Listen', bd=0, bg='white', command=self.recogniser, width=90, relief=RIDGE)
        self.recognise_button.place(x=0, y=650)

        # this is regarding the pen
        self.pen_size_scale_frame = LabelFrame(self.root, text='SIZE', bd=0, bg='white', font=('arial', 12, 'bold'), relief=RIDGE)
        self.pen_size_scale_frame.place(x=0, y=310, height=200, width=90)
        self.pen_size = Scale(self.pen_size_scale_frame, orient=VERTICAL,
                              from_=0, to=25, command=self.changeW, length=170)
        self.pen_size.set(1)
    ...
    def r_callback(self, event):
        ...
        self.canvas.bind("<Button-1>", self.set_cursor)
        # Change the name to 'listening'
        self.recognise_button['text'] = 'Listening..'
        self.recognise_button['state'] = 'disabled'
        p1 = multiprocessing.Process(
            target=record_to_file, args=('speech.wav', ))
        p1.start()
        while True:
            if p1.is_alive() == False:
                # Change the font to change font of the text that comes after recognition
                self.canvas.create_text(
                    self.ax, self.ay, anchor=NW, fill=self.color_fg, font="Arial 15", text=audio_to_text(),)
...
# Below are the colorcahnging functions
    ...
    def change_colord(self):
        self.color_fg = '#F4F27E'

    def change_colore(self):
        self.color_fg = '#FF69B4'
    ...
    def save(self):
        ...
        filename: str = asksaveasfilename(initialdir="/home", title="Select file", filetypes=(
            ('JPEG', ('*.jpg', '*.jpeg', '*.jpe', '*.jfif')), ('PNG','*.png'), ('PS', '*.ps'), ('BMP', ('*.bmp', '*.jdib')),('GIF', '*.gif'), ('PDF', '*.pdf'),))
        print(filename)
